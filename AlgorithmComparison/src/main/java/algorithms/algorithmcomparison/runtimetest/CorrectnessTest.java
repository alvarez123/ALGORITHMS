	...
	public void test() {
		try {
			...
			for (int i = 0; i < 10; i++) {
				...
				writer.write(String.format("Algorithm:\t\t%-20s%s",
						"Bubble Sort", "Merge Sort"));
				writer.newLine();
				writer.write(String.format("Running Time:\t\t%-20f%f",
						bubble.getAverageRuntime(), merge.getAverageRuntime()));
				writer.newLine();
				writer.write(String
						.format("Array size:\t\t%-20d%d", size, size));
				writer.newLine();
				writer.write(String.format("No. of key comparison:\t%-20f%f",
						bubble.getAverageKeyComparisons(),
						merge.getAverageKeyComparisons()));
				writer.newLine();
				writer.newLine();
				...
				for (int k = 0; k < 10; k++) {
					...
					for (int t = 0; t < size; t++) {
						writer.flush();
						writer.write(String.format("%d\t%d\t%d", inputarray[t],
								resultBubble[t], resultMerge[t]));
						writer.newLine();
					}
					writer.newLine();
				}
